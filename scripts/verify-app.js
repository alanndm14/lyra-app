...
for (const id of ['entryGate', 'appShell', 'chartList', 'playerOverlay', 'beatStage', 'mediaFallback', 'youtubeSearchLink']) {
  assert(ids.includes(id), `Missing required UI id: ${id}`);
}

assert(html.includes('styles.css?v=5') && html.includes('app.js?v=5'), 'Public asset versions are not aligned');
